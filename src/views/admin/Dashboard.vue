  ...
  <AdminLayout>
    ...
    <form action="javascript:" method="POST" class="max-w-3xl mx-auto pb-10">
      <div class="grid grid-cols-1 flex-row m-3 py-3 px-2 bg-white shadow rounded-xl">
        <input v-if="!isLoad" type="text" v-model="titleData" placeholder="Title" class="border col-span-4 border-neutral-400 focus:border-emerald-600 text-sm px-4 rounded-xl w-full p-2 outline-0 bg-white" />
        <div v-if="isLoad" class="h-10 w-full bg-gray-200 animate-pulse rounded-2xl">
      </div>
      </div>
    </form>
  </AdminLayout>
...
const originalTitleData = ref("");
const isDirty = ref(false);
const isLoading = ref(false);
const isLoad = ref(true)

onMounted(() => {
  const dbRefTitle = dbRef(db, "title");

  onValue(dbRefTitle, (snapshot) => {
    const data = snapshot.val();
    if (data) {
      titleData.value = data;
      originalTitleData.value = JSON.parse(JSON.stringify(data));
      isLoad.value = false
    }
  });
});
...
watch(
  titleData,
  (newVal) => {
    isDirty.value = !isEqual(newVal, originalTitleData.value);
  },
  { deep: true }
);
...
const saveTitle = async () => {
  ...
  try {
    await set(dbRef(db, "title"), titleData.value);

<template>
    <AdminLayout>
      ...
      <div class="max-w-3xl mx-auto pb-10">
        ...
        <div v-if="!isLoad" v-for="(item, index) in linksData" :key="index" class="grid grid-cols-1 md:grid-cols-12 md:gap-3 flex-row m-3 py-3 px-2 bg-white shadow rounded-xl">
          <input type="text" v-model="item.text" ref="linkInputs" placeholder="Website" class="border-b col-span-4 border-neutral-400 focus:border-emerald-600 text-sm px-4 rounded-xl w-full p-2 outline-0 bg-white" />
  ...
  const linkInputs = ref<(HTMLInputElement | null)[]>([]);
  ...
  const lastInput = linkInputs.value[linkInputs.value.length - 1];
  if (lastInput) {
    lastInput.focus();
  }
};
